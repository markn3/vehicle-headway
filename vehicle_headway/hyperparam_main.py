...
def main(expt_name, use_gpu, restart_opt, model_folder, hyperparam_iterations,
  ...
  with tf.Graph().as_default(), tf.compat.v1.Session(config=tf_config) as sess:
    ...
    p05_forecast = data_formatter.format_predictions(output_map["p5"])
    p50_forecast = data_formatter.format_predictions(output_map["p50"])
    p95_forecast = data_formatter.format_predictions(output_map["p95"])

    targets.to_csv("targets_2s_final.csv")
    p05_forecast.to_csv("p05_2s_final.csv")
    p50_forecast.to_csv("p50_2s_final.csv")
    p95_forecast.to_csv("p95_2s_final.csv")

    test_attention = model.get_attention(test)

    np.save('D:/Users/Mark Navalta/r/att/test_static_final', test_attention['static_flags'])

    np.save('D:/Users/Mark Navalta/r/att/test_historical_final', test_attention['historical_flags'])

    np.save('D:/Users/Mark Navalta/r/att/test_future_final', test_attention['future_flags'])
    ...
    p50_loss = utils.numpy_normalised_quantile_loss(
        ...
        0.5)
    p95_loss = utils.numpy_normalised_quantile_loss(
        extract_numerical_data(targets), extract_numerical_data(p95_forecast),
        0.95)
